...
def printHelp():
    ...
    print "          -c <directory> ------------------> Clean " + sys.argv[0] + " footprint about directory. '*' means every footprint."
...
def backup_directory_timestamps(target_dir,  data_dir):
    for root, dirs, files in os.walk(target_dir):
        for file in files:
            ...
            file_times.device = getFileSystem(file_path)
        ...
        for file in files:
                ...
                if (file_times.path == root):
                    if (file_times.filename == file):
                        if (((os.stat(file_path))[stat.ST_MTIME] != file_times.mtime) or ((os.stat(file_path))[stat.ST_ATIME] != file_times.atime) or ((os.stat(file_path))[stat.ST_CTIME] != file_times.ctime)):
                            print "Setting time of " + file_path + " from atime " + str((os.stat(file_path))[stat.ST_ATIME]) + ", atime " + str((os.stat(file_path))[stat.ST_MTIME]) + " and ctime " + str((os.stat(file_path))[stat.ST_CTIME])
                            print "                                   to atime " + str(file_times.atime) + ", mtime " + str(file_times.mtime) + " and ctime " + str(file_times.ctime)
            ...
            if (not file_found):
                if not (os.path.islink(os.path.join(root, file))):
                    print "ALERT:   The file " +  os.path.join(root, file) + " was not in our backup list, maybe it was created after,"
...
def changeCTime(filetimes):
    ...
    if not(ctime_change_error):
        print "sudo debugfs -w " + str(filetimes.device) + " -R \'set_inode_field <"+ str(filetimes.inode) + "> ctime " + str(filetimes.ctime) +"\'"
        try:
